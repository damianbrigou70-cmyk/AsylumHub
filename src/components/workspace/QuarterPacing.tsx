    ...
    <GlassPanel className="p-6">
      ...
      <div className="mt-5 space-y-2">
        <div className="relative h-1.5 w-full overflow-hidden rounded-full bg-white/[0.06]">
          <div
            className="absolute inset-y-0 left-0 rounded-full bg-gradient-to-r from-[oklch(0.64_0.22_25)] via-[oklch(0.7_0.2_15)] to-[oklch(0.72_0.19_35)] transition-all"
        ...
        <div className="flex items-center gap-4 text-[10px] uppercase tracking-wider text-muted-foreground">
          <span className="inline-flex items-center gap-1.5">
            <span className="inline-block h-1.5 w-3 rounded-full bg-gradient-to-r from-[oklch(0.64_0.22_25)] to-[oklch(0.72_0.19_35)]" />
            Complete
          </span>
          <span className="inline-flex items-center gap-1.5">
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <defs>
                {data.map((d) => (
                  <linearGradient
                    ...
                    y2="1"
                  >
                    <stop offset="0%" stopColor="oklch(0.72 0.19 35)" stopOpacity={1} />
                    <stop offset="55%" stopColor="oklch(0.7 0.2 12)" stopOpacity={0.95} />
                    <stop offset="100%" stopColor="oklch(0.64 0.22 25)" stopOpacity={0.85} />
                  </linearGradient>
                ))}
              </defs>
              ...
              <Bar dataKey="count" radius={[6, 6, 0, 0]} maxBarSize={56}>
                {data.map((d) => (
                  <Cell key={d.status} fill={`url(#bar-${d.status})`} stroke="oklch(0.7 0.2 12)" strokeOpacity={0.4} />
                ))}
                <LabelList
                  dataKey="count"
                  position="top"
