...
function radialPoint(index: number, total: number, radiusX: number, radiusY: number) {
  const angle = -Math.PI / 2 + (Math.PI * 2 * index) / total;
    return {
      x: CX + Math.cos(angle) * radiusX,
      y: RADIAL_CY + Math.sin(angle) * radiusY,
    };
}

function radialSatellitePoint(primary: Primary, satellite: Satellite) {
  const realSatellites = (item: Primary) => item.satellites.filter((entry) => entry.id !== ADD_SATELLITE_ID);
  const index = PRIMARIES
    .slice(0, PRIMARIES.indexOf(primary))
    .reduce((count, item) => count + realSatellites(item).length, 0) + realSatellites(primary).findIndex((entry) => entry.id === satellite.id);
  const total = PRIMARIES.reduce((count, item) => count + realSatellites(item).length, 0);
  const perRing = 12;
  const ring = Math.floor(index / perRing);
  const ringIndex = index % perRing;
    const ringTotal = Math.min(perRing, total - ring * perRing);
    return radialPoint(ringIndex, ringTotal, 380 + ring * 105, 300 + ring * 75);
}
  ...
  const defs: Array<Omit<Primary, "satellites"> & { sats: SatelliteSpec[] }> = [
    {
      ...
      sats: [
        { id: "utm-all", label: "UAV Tracking", to: "/tools/all-utms", icon: <IconScroll size={20} /> },
        { id: "name", label: "PVP Killfeed", to: "/tools/utm", icon: <IconSpark size={20} /> },
        { id: "tax", label: "Bounties", to: "/tools/taxonomy", icon: <IconSpark size={20} /> },
        { id: "counter", label: "Counter-UAV", to: "/tools/list-cleaner", icon: <IconSpark size={20} /> },
        { id: "board", label: "Leaderboards", to: "/tools/campaign-performance", icon: <IconChart size={20} /> },
        ADD_SAT,
      ],
    },
    {
      ...
      sats: [
        { id: "targets", label: "Precision Strikes", to: "/tools/funnel-targets", icon: <IconSpark size={20} /> },
        { id: "perf2", label: "Bomb Strafe Run", to: "/tools/campaign-performance", icon: <IconChart size={20} /> },
        { id: "gas", label: "Gas Strafe Run", to: "/tools/campaign-performance", icon: <IconChart size={20} /> },
        { id: "targeting", label: "Interactive Targeting", to: "/funnel", icon: <IconSpark size={20} /> },
        ADD_SAT,
      ],
    },
    {
      ...
      sats: [
        { id: "creator", label: "Base Protection", to: "/tools/campaign-creator", icon: <IconCampaign size={20} /> },
        { id: "events", label: "Raid Announcements", to: "/tools/events", icon: <IconCalendar size={20} /> },
        { id: "bases", label: "Custom Bases", to: "/tools/campaign-in-a-box", icon: <IconCampaign size={20} /> },
        ADD_SAT,
      ],
    },
    {
      ...
      r: 2,
      hue: 285,
      icon: <IconImport size={34} />,
      sats: [
        { id: "npc", label: "NPC Maker", to: "/tools/import", icon: <IconImport size={20} /> },
        { id: "vehicles", label: "Custom Vehicles", to: "/tools/event-intake", icon: <IconSpark size={20} /> },
        { id: "shop", label: "Item Shop", to: "/templates", icon: <IconScroll size={20} /> },
        ADD_SAT,
      ],
    },
    {
      ...
      r: 4,
      hue: 145,
      icon: <IconSpark size={34} />,
      sats: [
        { id: "faction-join", label: "Create or Join", to: "/workspaces", icon: <IconSpark size={20} /> },
        { id: "faction-wars", label: "Wars & Contracts", to: "/campaigns", icon: <IconCampaign size={20} /> },
        { id: "faction-board", label: "Faction Leaderboard", to: "/leads", icon: <IconChart size={20} /> },
        ADD_SAT,
      ],
    },
    {
      ...
      sats: [
        { id: "credits", label: "Credits Economy", to: "/leads", icon: <IconChart size={20} /> },
        { id: "server-events", label: "Battlepass", to: "/battlepass", icon: <IconCalendar size={20} /> },
        { id: "xp", label: "XP & Levels", to: "/tools/campaign-performance", icon: <IconSpark size={20} /> },
        { id: "tickets", label: "Support Tickets", to: "/requests", icon: <IconScroll size={20} /> },
        ADD_SAT,
      ],
    },
    {
...
const LOBBY_ONLY_IDS = new Set(["online", "operations", "server-events", "support"]);
...
interface HexToolsTreeProps {
  ...
  /** Uses a radial layout for the Lobby launcher instead of the honeycomb. */
  radial?: boolean;
}
...
export function HexToolsTree({
  ...
  const [accountReady, setAccountReady] = useState(() => accountLinksComplete(readAccountLinks()));
  const hidden = useHiddenHexes();
  const satelliteOrders = useSatelliteOrders();

  useEffect(() => {
    const sync = () => setAccountReady(accountLinksComplete(readAccountLinks()));
    window.addEventListener("asylumhub:account-links-updated", sync);
    window.addEventListener("storage", sync);
    return () => {
      window.removeEventListener("asylumhub:account-links-updated", sync);
  ...
  return (
    <div
      className={`mp-htt-root${isFocusMode ? " is-focus-mode" : ""}${editMode ? " is-edit-mode" : ""}${radial ? " is-radial" : ""}`}
      onMouseLeave={() => {
        setHoverId(null);
        setTapRevealed(null);
      }}
    >
      ...
      <div className="mp-htt-scaler" style={{ width: editMode ? EDIT_STAGE_W : STAGE_W, height: editMode ? EDIT_STAGE_H : STAGE_H }}>
        ...
        >
          ...
          {radial && (
            <div className="mp-htt-fire-field" aria-hidden>
              <div className="mp-htt-smoke mp-htt-smoke-one" />
              <div className="mp-htt-smoke mp-htt-smoke-two" />
              <div className="mp-htt-smoke mp-htt-smoke-three" />
              {Array.from({ length: 90 }, (_, index) => (
                <span
                  key={`spark-${index}`}
                  className="mp-htt-fire-spark"
                  style={{
                    ["--spark-x" as string]: `${8 + ((index * 37) % 84)}%`,
                    ["--spark-delay" as string]: `${(index % 9) * -0.42}s`,
                    ["--spark-duration" as string]: `${3.4 + (index % 5) * 0.55}s`,
                    ["--spark-size" as string]: `${2 + (index % 3)}px`,
                  }}
                />
              ))}
            </div>
          ...
          {radial && (
            <div className="mp-htt-radial-center" aria-hidden>
              <span className="mp-htt-radial-kicker">ASYLUMHUB // DAYZ SERVER</span>
              <strong>Choose your next move.</strong>
              <span>Survive. Build. Conquer.</span>
            </div>
          )}
          ...
          {!editMode && !radial && (
            <div className="mp-htt-ghost-layer" aria-hidden>
              {GHOST_CELLS.filter((c) => fadeGhostEdges || (c.r >= -1 && c.r <= 5)).map((c) => {
          ...
          {PRIMARIES.map((p) => {
            ...
            if (!radial && LOBBY_ONLY_IDS.has(p.id)) return null;
            if (hidden.has(p.id) && !radial) return null;
            ...
            const pos =
              editMode && editBandIndex >= 0
                ...
                : radial
                  ? radialPoint(PRIMARIES.indexOf(p), PRIMARIES.length, 270, 270)
                  : axial(p.q, p.r);
            ...
            return (
              ...
              >
                ...
                {!accountReady && <span className="mp-htt-lock"><LockKeyhole size={18} /></span>}
                {editMode && <EditRing />}
                {editMode && (
                  <span
                    role="button"
          ...
          {/* Keep every shortcut visible in the Lobby hub. Hover still
              highlights a primary, but no longer controls whether its
              child buttons exist. */}
          {showAllSatellites &&
            PRIMARIES.filter((p) => !hidden.has(p.id)).flatMap((p) =>
              orderedSatellites(p)
                .filter((s) => s.id !== ADD_SATELLITE_ID && !hidden.has(satKey(p.id, s.id)))
                .map((s) => {
                  const pos = radialSatellitePoint(p, s);
                  return (
                    <div
                      key={`always-${p.id}-${s.id}`}
                      className="mp-htt-sat-anchor"
                      style={{
                        left: pos.x,
                        top: pos.y,
                    ...
                    >
                      <button
                        type="button"
                        className="mp-htt-hex mp-htt-sat mp-htt-sat-inner"
                        style={{ width: HEX_W, height: HEX_H, ["--hex-hue" as string]: p.hue }}
                        onClick={(e) => {
                          e.stopPropagation();
                          ...
                          if (onSatelliteFocus?.(p.id, s.id)) return;
                          if (!onSatelliteFocus) go(s.to);
                        }}
                        aria-label={s.id === ADD_SATELLITE_ID ? `Add custom hex to ${p.name}` : s.label}
                      >
                        <span className="mp-htt-sheen" aria-hidden />
          ...
          <AnimatePresence>
            {activePrimary && !showAllSatellites && !radial && orderedSatellites(activePrimary)
              .filter(
                (s) =>
              ...
              .map((s, i) => {
              const pos = radial ? radialSatellitePoint(activePrimary, s) : axial(s.q, s.r);
...
.mp-htt-stars {
  ...
  background-image:
    ...
    radial-gradient(1px 1px at 78% 18%, oklch(0.74 0.19 25 / 0.6), transparent 60%),
    ...
    radial-gradient(1px 1px at 50% 50%, oklch(0.72 0.19 35 / 0.35), transparent 60%);
  animation: mp-htt-twinkle 9s ease-in-out infinite;
}
.mp-htt-radial-center {
  position: absolute;
  left: ${CX}px;
  top: ${RADIAL_CY}px;
  z-index: 1;
  ...
  text-align: center;
  pointer-events: none;
  color: oklch(0.92 0.02 25 / 0.78);
}
.mp-htt-radial-center strong {
  font-family: var(--font-display);
  font-size: 22px;
  line-height: 1.05;
  letter-spacing: 0.01em;
  color: oklch(0.98 0.02 25 / 0.92);
}
.mp-htt-radial-center > span:last-child {
  font-size: 11px;
  letter-spacing: 0.2em;
  text-transform: uppercase;
  color: oklch(0.78 0.08 25 / 0.7);
}
.mp-htt-fire-field {
  position: absolute;
  inset: 0;
...
.mp-htt-smoke {
  ...
  border-radius: 50%;
  background:
    radial-gradient(ellipse at 38% 92%, oklch(0.14 0.04 25 / 0.34), transparent 36%),
    radial-gradient(ellipse at 65% 70%, oklch(0.2 0.03 270 / 0.24), transparent 40%),
    radial-gradient(ellipse at 42% 45%, oklch(0.16 0.03 280 / 0.2), transparent 38%),
    radial-gradient(ellipse at 60% 18%, oklch(0.2 0.02 280 / 0.15), transparent 34%);
  filter: blur(24px);
  opacity: 0.9;
  animation: mp-htt-smoke-rise 10s ease-in-out infinite;
}
.mp-htt-smoke-two {
  width: 210px;
  height: 540px;
...
.mp-htt-fire-spark {
  ...
  border-radius: 50%;
  background: oklch(0.99 0.2 75);
  box-shadow: 0 0 12px 3px oklch(0.82 0.24 35 / 0.95);
  opacity: 0;
  animation: mp-htt-spark-rise var(--spark-duration) ease-out var(--spark-delay) infinite;
}
@keyframes mp-htt-spark-rise {
  0% { opacity: 0; transform: translate3d(0, 20px, 0) scale(0.6); }
  12% { opacity: 0.9; }
  72% { opacity: 0.65; }
  100% { opacity: 0; transform: translate3d(18px, -460px, 0) scale(0.15); }
}
@keyframes mp-htt-smoke-rise {
  0%, 100% { transform: translate(-50%, 24px) scale(0.94, 0.98); opacity: 0.42; }
  50% { transform: translate(-44%, -24px) scale(1.08, 1.04); opacity: 0.75; }
}
@media (prefers-reduced-motion: reduce) {
  .mp-htt-fire-spark,
  .mp-htt-smoke { animation: none; }
}
.mp-htt-radial-kicker {
  font-family: var(--font-display);
  font-size: 9px;
  letter-spacing: 0.22em;
  text-transform: uppercase;
  color: oklch(0.75 0.14 25 / 0.72);
}
...
.mp-htt-hex:hover::after {
  box-shadow:
    ...
    0 24px 60px -10px oklch(0.06 0.02 25 / 0.95),
...
.mp-htt-primary.is-active .mp-htt-rule {
  background: linear-gradient(90deg, transparent, oklch(0.8 0.16 55 / 0.9), transparent);
}
...
.mp-htt-root.is-radial .mp-htt-primary { z-index: 8; }
.mp-htt-root.is-radial .mp-htt-sat-inner {
  transform: scale(0.5);
}
...
.mp-htt-root.is-focus-mode .mp-htt-sat::after {
  ...
  box-shadow:
    ...
    0 20px 50px -12px oklch(0.06 0.02 25 / 0.95);
}
...
.mp-htt-lock {
  ...
  transform: translateX(-50%);
  pointer-events: none;
  color: oklch(0.98 0.01 20 / 0.9);
  filter: drop-shadow(0 2px 5px oklch(0 0 0 / 0.95));
}
.mp-htt-primary:disabled {
  cursor: not-allowed;
  filter: brightness(0.3) saturate(0.25);
}
.mp-htt-primary:disabled::before {
  background: oklch(0.3 0.04 var(--hex-hue, 88) / 0.42);
}
.mp-htt-primary:disabled::after {
  background: linear-gradient(160deg,
    oklch(0.12 0.025 var(--hex-hue, 290) / 0.98) 0%,
    oklch(0.075 0.015 320 / 0.99) 55%,
    oklch(0.025 0.008 25 / 1) 100%);
  box-shadow: inset 0 0 30px oklch(0 0 0 / 0.7), 0 14px 36px -14px oklch(0 0 0 / 0.95);
}
