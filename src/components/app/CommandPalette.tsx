...
export function CommandPalette() {
  ...
  return (
    <CommandDialog open={open} onOpenChange={setOpen}>
      <CommandInput placeholder="Search factions, services, players, strikes…" />
      <CommandList>
        ...
        <CommandGroup heading="Quick">
          ...
          <CommandItem onSelect={() => go(() => nav({ to: "/campaigns" }))}>
            <IconWorkspace size={16} /> Operations
        ...
        <CommandGroup heading="Tools">
          <CommandItem onSelect={() => go(() => nav({ to: "/tools", search: { focus: "utm-campaign-name" } }))}>
            <IconCampaign size={16} /> Base Protection
          </CommandItem>
        ...
        {campaigns.length > 0 && (
          <>
            <CommandSeparator />
            <CommandGroup heading="Saved factions">
              {campaigns.map((c) => (
                <CommandItem
                  key={c.id}
        ...
        {utm.length > 0 && (
          <>
            <CommandSeparator />
            <CommandGroup heading="UAV targets">
              {utm.map((u) => (
                <CommandItem
                  key={u.id}
