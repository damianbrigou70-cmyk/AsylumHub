...
type TabId = "organization" | "team" | "account" | "server-chat";
const TAB_IDS: TabId[] = ["organization", "team", "account", "server-chat"];
    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-wrap gap-1 rounded-2xl border border-glass-border bg-glass/40 p-1">
        {[
          ...
          { id: "server-chat", label: "Server Chat", Icon: IconMessageSquare },
        ].map(({ id, label, Icon }) => {
          const active = tab === id;
          return (
            <button
...
function ServerChatTab() {
  const [message, setMessage] = useState("");
  const [messages, setMessages] = useState<{ author: string; body: string; time: string }[]>([]);

  const sendMessage = () => {
    const body = message.trim();
    if (!body) return;
    setMessages((current) => [...current, { author: "You", body, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) }]);
    setMessage("");
  };
    ...
    <GlassPanel className="overflow-hidden">
      <div className="border-b border-glass-border p-6">
        <div className="flex items-center gap-3">
          <div className="grid size-10 place-items-center rounded-xl bg-primary/15 text-primary"><IconMessageSquare size={19} /></div>
          <div><div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Server communication</div><h2 className="font-display text-2xl">Server Chat</h2></div>
        </div>
        <p className="mt-3 text-sm text-muted-foreground">Talk with your server community and coordinate what happens next.</p>
      </div>
      <div className="min-h-72 space-y-3 p-6">
        {messages.length === 0 ? <div className="grid min-h-56 place-items-center rounded-xl border border-dashed border-glass-border text-sm text-muted-foreground">No messages yet. Start the conversation.</div> : messages.map((entry, index) => <div key={`${entry.time}-${index}`} className="flex gap-3 rounded-xl bg-glass/30 p-3"><div className="grid size-8 shrink-0 place-items-center rounded-full bg-primary/15 text-xs text-primary">Y</div><div><div className="flex items-center gap-2 text-xs"><strong>{entry.author}</strong><span className="text-muted-foreground">{entry.time}</span></div><p className="mt-1 text-sm text-foreground/85">{entry.body}</p></div></div>)}
      </div>
      <div className="flex gap-2 border-t border-glass-border p-4">
        <input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => event.key === "Enter" && sendMessage()} placeholder="Write a message..." className="min-w-0 flex-1 rounded-xl border border-glass-border bg-black/20 px-4 py-3 text-sm outline-none placeholder:text-muted-foreground focus:border-primary/50" />
        <button type="button" onClick={sendMessage} className="rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90">Send</button>
      </div>
    </GlassPanel>
  );
}
