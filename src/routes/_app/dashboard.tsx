...
function Dashboard() {
  ...
  const avatarUrl =
    ...
    null;
  const avatarInitial = discordName.slice(0, 1).toUpperCase();

  return (
    <div className="space-y-10">
      <div className="flex flex-col items-center text-center" data-tour="dashboard-hero">
        <PageHexBadge
          hue={88}
          size={38}
          icon={
            avatarUrl ? (
              <img
                src={avatarUrl}
                alt={`${discordName} avatar`}
                className="size-12 rounded-full object-cover ring-2 ring-amber-200/70"
              />
            ) : (
              <span className="text-2xl font-semibold text-amber-100">{avatarInitial}</span>
            )
          }
          aria-label={`${discordName} profile`}
        />
        <div className="mt-4 flex flex-col items-center">
          ...
          <h1 className="mt-1 font-display text-4xl tracking-tight">
            {isFirstVisit
              ? `Welcome, ${discordName}.`
              : `Welcome back, ${discordName}.`}
          </h1>
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">
            {isFirstVisit
              ? "Your command center is online. Link your survivor, scan the tools, and take control of the frontier."
              : "What needs you, how you're pacing, and what's coming up."}
          </p>
        </div>
        <button
          type="button"
          onClick={() => document.getElementById("account-link-setup")?.scrollIntoView({ behavior: "smooth", block: "center" })}
          className="dashboard-link-account mt-5 inline-flex items-center gap-2 rounded-xl border border-primary/40 bg-primary/10 px-5 py-2.5 text-sm font-medium text-primary transition hover:border-primary/70 hover:bg-primary/20"
        >
          Link account <IconArrowRight size={15} />
        </button>
      </div>

      <style>{`
        .dashboard-link-account {
          animation: dashboard-link-pulse 2.2s ease-in-out infinite;
        }
        @keyframes dashboard-link-pulse {
          0%, 100% {
            box-shadow: 0 0 0 0 oklch(0.78 0.18 75 / 0.22);
          }
          50% {
            box-shadow: 0 0 0 8px oklch(0.78 0.18 75 / 0);
          }
        }
        @media (prefers-reduced-motion: reduce) {
          .dashboard-link-account { animation: none; }
        }
      `}</style>
