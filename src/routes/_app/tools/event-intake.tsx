...
export function FactionHubContent() {
  ...
  const [createOpen, setCreateOpen] = useState(false);
  const [tab, setTab] = useState<"roster" | "wars" | "directory">("roster");
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-primary/20 pb-5">
        <div className="flex items-center gap-4">
          <PageHexBadge hue={45} icon={<IconCampaign size={26} />} aria-label="Faction Hub" />
          <div>
            <div className="text-[11px] uppercase tracking-[0.28em] text-primary">Faction command</div>
            <h1 className="shop-medieval-title mt-1 text-3xl text-primary md:text-4xl">Faction Hub</h1>
          </div>
        </div>
        {faction && (
          <div className="flex items-center gap-3 rounded-xl border border-primary/30 bg-black/50 px-4 py-2.5">
            <img src={flagImage(getFactionFlag(faction.flag)[2])} alt={getFactionFlag(faction.flag)[1]} className="size-9 rounded-lg border border-primary/40 bg-primary/10 p-1 object-contain" />
            <div className="text-xs">
              <div className="font-medium text-foreground">{faction.name}</div>
              <div className="text-muted-foreground">{faction.members.length} members · Map {faction.map}</div>
            </div>
          </div>
        )}
      </header>
      ...
      {!faction ? (
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
          <div className="rounded-2xl border border-primary/25 bg-black/60 p-8 text-center">
            ...
            <h2 className="shop-medieval-title mt-5 text-2xl text-primary">No faction created</h2>
            ...
            {!createOpen ? (
              <button type="button" onClick={() => setCreateOpen(true)} className="mt-6 rounded-lg bg-primary px-6 py-3 text-sm font-semibold uppercase tracking-wider text-primary-foreground hover:brightness-110">Create faction</button>
            ) : (
              <div className="mx-auto mt-6 max-w-md space-y-4 text-left">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {(["101x", "102x"] as const).map((mapId) => (
                      <button key={mapId} type="button" onClick={() => setMap(mapId)} className={`rounded-lg border px-3 py-3 text-left transition ${map === mapId ? "border-primary bg-primary/20 text-foreground" : "border-white/10 text-muted-foreground hover:bg-white/5"}`}><span className="block font-mono text-sm">{mapId}</span><span className="mt-1 block text-[10px]">{mapId === "101x" ? "Livonia" : "Chernarus"}</span></button>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between rounded-lg border border-primary/30 bg-primary/10 px-4 py-3 text-sm">
                  ...
                  <strong className="font-mono text-primary">10,000 credits</strong>
                </div>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setCreateOpen(false)} className="flex-1 rounded-lg border border-white/10 px-4 py-3 text-sm text-muted-foreground hover:bg-white/5">Cancel</button>
          ...
          <div className="rounded-2xl border border-white/10 bg-black/40 p-6 opacity-75">
            <div className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground">Locked until you create a faction</div>
            <h2 className="mt-2 text-xl text-foreground">War Room</h2>
            <p className="mt-2 text-sm text-muted-foreground">Declare wars, track conflicts, and call ceasefires.</p>
          </div>
        </div>
      ) : (
        <div className="grid gap-5 lg:grid-cols-[240px_minmax(0,1fr)]">
          <aside className="space-y-3 rounded-2xl border border-primary/25 bg-black/60 p-4 lg:h-fit lg:sticky lg:top-24">
            <div className="text-center">
              <img src={flagImage(getFactionFlag(faction.flag)[2])} alt={getFactionFlag(faction.flag)[1]} className="mx-auto size-16 rounded-2xl border border-primary/40 bg-primary/10 p-2 object-contain" />
              <h2 className="shop-medieval-title mt-3 text-xl text-primary">{faction.name}</h2>
              <p className="mt-1 text-[11px] text-muted-foreground">Map {faction.map}</p>
            </div>
            <div className="grid grid-cols-3 gap-2 border-y border-white/10 py-3 text-center">
              <div><div className="font-mono text-sm text-foreground">{faction.members.length}</div><div className="text-[9px] uppercase text-muted-foreground">Members</div></div>
              <div><div className="font-mono text-sm text-foreground">{faction.credits.toLocaleString()}</div><div className="text-[9px] uppercase text-muted-foreground">Credits</div></div>
              <div><div className="font-mono text-sm text-primary">{faction.reputation.toLocaleString()}</div><div className="text-[9px] uppercase text-muted-foreground">Reputation</div></div>
            </div>
            <nav className="space-y-1">
              {([
                ...
                <button key={id} type="button" onClick={() => setTab(id)} className={`block w-full rounded-lg px-3 py-2 text-left text-sm transition ${tab === id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-white/5 hover:text-foreground"}`}>{label}</button>
              ))}
            </nav>
            <div className="flex flex-col gap-1.5 border-t border-white/10 pt-3">
              <button type="button" onClick={() => setEditOpen((current) => !current)} className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-muted-foreground hover:bg-white/5 hover:text-foreground">{editOpen ? "Close settings" : "Edit faction"}</button>
                  ...
                  <div className="rounded-2xl border border-primary/25 bg-black/60 p-6">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        ...
                        <h3 className="mt-1 text-xl text-foreground">Faction settings</h3>
                      </div>
              ...
              <div className="rounded-2xl border border-primary/25 bg-black/60 p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-xl text-foreground">Manage members</h3>
                  </div>
                  <span className="text-xs text-primary">{faction.members.length} active</span>
                </div>
                <div className="mt-4 space-y-2">
                  {faction.members.length === 0 && <p className="rounded-lg border border-dashed border-white/10 p-4 text-sm text-muted-foreground">No members yet. Add your first survivor below.</p>}
                  {faction.members.map((member) => (
                    <div key={member} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.03] px-3 py-2 text-sm">
                      <span>{member}</span>
            ...
            {tab === "wars" && (
              <div className="rounded-2xl border border-primary/25 bg-black/60 p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="text-xs uppercase tracking-[0.18em] text-primary">Faction operations</div>
                    <h3 className="mt-1 text-2xl text-foreground">War Room</h3>
                  ...
                  <div className="rounded-lg bg-primary/10 px-3 py-2 text-right"><div className="text-[10px] uppercase tracking-wider text-primary">Active conflicts</div><div className="font-mono text-xl text-foreground">{wars.filter((war) => war.status === "active").length}</div></div>
                </div>
                <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(220px,.7fr)]">
                  <div className="space-y-2">
                    {wars.length === 0 && <div className="rounded-xl border border-dashed border-white/10 bg-black/20 p-5 text-sm text-muted-foreground">No conflicts on the board. Choose a faction to open hostilities.</div>}
                    {wars.map((war) => (
                      <div key={war.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-primary/20 bg-primary/5 px-4 py-3">
                        <div><div className="text-sm font-medium text-foreground">{faction.name} <span className="text-primary">vs.</span> {war.target}</div><div className="mt-1 text-[11px] text-muted-foreground">Declared {war.startedAt} · {war.status === "active" ? "Hostilities active" : "Ceasefire signed"}</div></div>
                        {war.status === "active" && <button type="button" onClick={() => { setWars((current) => current.map((entry) => entry.id === war.id ? { ...entry, status: "ceasefire" } : entry)); toast.success("Ceasefire signed", { description: `The conflict with ${war.target} is paused.` }); }} className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-muted-foreground hover:border-emerald-300/50 hover:text-emerald-200">Call ceasefire</button>}
                      </div>
                    ))}
                  </div>
                  <div className="rounded-xl border border-white/10 bg-black/30 p-4">
                    <label className="text-xs uppercase tracking-wider text-muted-foreground">Choose target</label>
                    <select value={warTarget} onChange={(event) => setWarTarget(event.target.value)} className="mt-2 w-full rounded-lg border border-white/10 bg-black/50 px-3 py-2.5 text-sm text-foreground outline-none">
                    ...
                    <button type="button" onClick={declareWar} disabled={wars.some((war) => war.target === warTarget && war.status === "active")} className="mt-3 w-full rounded-lg border border-primary/50 bg-primary/15 px-4 py-2.5 text-sm font-medium text-primary hover:bg-primary/25 disabled:cursor-not-allowed disabled:opacity-40">Declare war</button>
                  </div>
                </div>
                <div className="mt-5 border-t border-white/5 pt-5">
                  <div className="flex flex-wrap items-end justify-between gap-3"><div><div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Faction reputation</div><div className="mt-1 text-3xl text-primary">{faction.reputation.toLocaleString()}</div></div><span className="text-xs text-muted-foreground">Earned by actions across the server</span></div>
                  <div className="mt-3 grid gap-2 sm:grid-cols-3">
                    <button type="button" onClick={() => earnReputation(25, "Player kill recorded")} className="rounded-lg bg-white/5 px-3 py-3 text-left transition hover:bg-white/10"><span className="block text-sm text-foreground">Record a kill</span><span className="mt-1 block text-xs text-primary">+25 reputation</span></button>
                    <button type="button" onClick={() => earnReputation(100, "Contract completed")} className="rounded-lg bg-white/5 px-3 py-3 text-left transition hover:bg-white/10"><span className="block text-sm text-foreground">Complete contract</span><span className="mt-1 block text-xs text-primary">+100 reputation</span></button>
                    <button type="button" onClick={() => earnReputation(150, "Territory defense logged")} className="rounded-lg bg-white/5 px-3 py-3 text-left transition hover:bg-white/10"><span className="block text-sm text-foreground">Defend territory</span><span className="mt-1 block text-xs text-primary">+150 reputation</span></button>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="rounded-2xl border border-primary/25 bg-black/60 p-6">
                <div className="flex flex-wrap items-end justify-between gap-3">
                  <div>
                    ...
                    <h2 className="mt-1 text-2xl text-foreground">Find a faction</h2>
                    <p className="mt-1 text-sm text-muted-foreground">Join an existing faction or invite them to yours.</p>
                  </div>
                  <span className="text-xs text-muted-foreground">{SERVER_FACTIONS.length} active factions</span>
                </div>
                <div className="mt-4 grid gap-2 md:grid-cols-2">
                  {SERVER_FACTIONS.map((serverFaction) => {
                    const requested = joinRequests.includes(serverFaction.name);
                    return (
                      <div key={serverFaction.name} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-3">
                        ...
                        <button type="button" disabled={requested} onClick={() => setJoinRequests((current) => [...current, serverFaction.name])} className="shrink-0 rounded-lg border border-white/10 px-3 py-1.5 text-xs text-muted-foreground hover:bg-white/5 disabled:cursor-default disabled:text-emerald-300">
