...
const CONTRACTS = [
  { name: "Bounty Contract", description: "Hunt down a specific player or faction member and bring back proof of completion." },
  { name: "Arms Dealer", description: "Retrieve a weapons crate from a dangerous location." },
  { name: "Treasure Hunt", description: "Follow clues across the map to locate a hidden reward." },
  { name: "Last Stand", description: "Hold a designated location for a set amount of time." },
  { name: "Wasteland Survey", description: "Visit 5 locations and document what you find." },
  { name: "Assassination", description: "Eliminate a designated target." },
];
...
function timeRemaining(expiresAt: string, currentTime: number) {
  const remaining = Math.max(0, new Date(expiresAt).getTime() - currentTime);
  const hours = Math.floor(remaining / 3_600_000);
  const minutes = Math.floor((remaining % 3_600_000) / 60_000);
  return remaining === 0 ? "Expired" : `${hours}h ${minutes.toString().padStart(2, "0")}m remaining`;
}
...
function WorkspacesList() {
  ...
  const [lastAccepted, setLastAccepted] = useState<ActiveContract | null>(null);
  const [activeContractsOpen, setActiveContractsOpen] = useState(false);
  const [currentTime, setCurrentTime] = useState(() => Date.now());
  const draft = useServerFn(draftCampaignFromBrief);

  useEffect(() => {
    if (!orgId) return;
    const storageKey = `asylumhub:active-contracts:${orgId}`;
    try {
      const stored = JSON.parse(localStorage.getItem(storageKey) ?? "[]") as ActiveContract[];
      setActiveContracts(stored.filter((contract) => new Date(contract.expiresAt).getTime() > Date.now()));
    } catch {
      setActiveContracts([]);
    }
  ...
  useEffect(() => {
    if (!orgId) return;
    const active = activeContracts.filter((contract) => new Date(contract.expiresAt).getTime() > currentTime);
    if (active.length !== activeContracts.length) {
      setActiveContracts(active);
      localStorage.setItem(`asylumhub:active-contracts:${orgId}`, JSON.stringify(active));
    }
  }, [activeContracts, currentTime, orgId]);
  ...
  const create = async () => {
    if (!selectedContract) {
      toast.message("Select a contract before deploying");
      return;
    }
    if (!orgId || !user) {
    ...
    setCreating(true);
    const now = new Date();
    const contract = CONTRACTS.find((item) => item.name === selectedContract);
    const draftName = `${selectedContract} — ${now.toLocaleDateString(undefined, {
      month: "short",
      day: "numeric",
    })}`;
    const { data, error } = await supabase
      ...
      .insert({
        ...
        name: draftName,
        status: "draft",
        goal: contract?.description ?? null,
      })
      .select("id")
      .single();
    if (error || !data) {
    ...
    const accepted: ActiveContract = {
      id: crypto.randomUUID(),
      workspaceId: data.id,
      name: selectedContract,
      description: contract?.description ?? "",
      acceptedAt: acceptedAt.toISOString(),
      expiresAt: new Date(acceptedAt.getTime() + 4 * 60 * 60 * 1000).toISOString(),
    };
    const nextActive = [accepted, ...activeContracts];
    setActiveContracts(nextActive);
    setLastAccepted(accepted);
    setSelectedContract(null);
    localStorage.setItem(`asylumhub:active-contracts:${orgId}`, JSON.stringify(nextActive));
    ...
    <div className="space-y-8">
      <div className="flex items-end justify-between gap-4">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            onClick={() => setActiveContractsOpen(true)}
            disabled={activeContracts.length === 0}
            className="inline-flex items-center gap-2 rounded-full border border-glass-border bg-glass/30 px-4 py-2.5 text-sm hover:bg-glass/50 disabled:cursor-not-allowed disabled:opacity-50"
          >
          ...
          >
            <IconPlus size={16} />
            {creating ? "Creating…" : "Deploy Contract"}
          </button>
        </div>
      </div>
      ...
      {activeContractsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setActiveContractsOpen(false)}>
          <GlassPanel tier="strong" className="relative w-full max-w-lg p-6" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setActiveContractsOpen(false)}
              className="absolute right-3 top-3 text-muted-foreground hover:text-foreground"
              aria-label="Close active contracts"
            >
              <IconClose size={16} />
            </button>
            <h2 className="font-display text-xl">Active Contracts</h2>
            <div className="mt-4 space-y-3">
              {activeContracts.map((contract) => (
                <div key={contract.id} className="rounded-xl border border-glass-border bg-glass/30 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="shrink-0 font-mono text-[10px] uppercase tracking-wider text-primary">
                      {timeRemaining(contract.expiresAt, currentTime)}
                    </span>
                  </div>
                  <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{contract.description}</p>
                  <p className="mt-3 text-[10px] uppercase tracking-wider text-muted-foreground/70">
                    Accepted {new Date(contract.acceptedAt).toLocaleString()}
                  </p>
                </div>
              ))}
            </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h2 className="font-display text-lg">Available contracts</h2>
          <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">6 contracts</span>
        </div>
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {CONTRACTS.map((contract) => (
            <button
              ...
              aria-pressed={selectedContract === contract.name}
              className={`rounded-xl border px-4 py-3 text-left transition ${
                selectedContract === contract.name
                  ? "border-primary/70 bg-primary/15 shadow-[0_8px_30px_-12px_oklch(0.78_0.2_280/0.5)]"
                  : "border-glass-border bg-glass/30 hover:border-primary/40 hover:bg-glass/50"
              }`}
            >
              <h3 className="text-sm font-medium">{contract.name}</h3>
              <p className="mt-1 text-xs leading-relaxed text-muted-foreground">{contract.description}</p>
            </button>
          ))}
        </div>
        <p className="text-xs text-muted-foreground">
          {selectedContract ? `Selected: ${selectedContract}. Click Deploy Contract to accept it.` : "Select a contract, then click Deploy Contract to accept it."}
        </p>
      </div>

      {lastAccepted && (
        <GlassPanel tier="strong" className="border-primary/40 p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-primary">Contract accepted</p>
              <h2 className="mt-1 font-display text-2xl">{lastAccepted.name}</h2>
              <p className="mt-2 max-w-2xl text-sm text-muted-foreground">{lastAccepted.description}</p>
            </div>
            <div className="rounded-lg border border-primary/30 bg-primary/10 px-3 py-2 text-right">
              <div className="font-mono text-lg text-primary">{timeRemaining(lastAccepted.expiresAt, currentTime)}</div>
              <div className="text-[10px] uppercase tracking-wider text-muted-foreground">4-hour contract timer</div>
            </div>
          </div>
          <p className="mt-4 text-xs text-muted-foreground">
            Accepted {new Date(lastAccepted.acceptedAt).toLocaleString()}. Complete the objective before the timer expires.
          </p>
        </GlassPanel>
      )}
        ...
        <GlassPanel tier="strong" className="flex flex-col gap-8 p-10">
          ...
          <div className="flex flex-col items-center gap-2">
            <div className="flex flex-wrap items-center justify-center gap-2">
              <button onClick={create} disabled={creating || !selectedContract} className="inline-flex items-center gap-2 rounded-full bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">
                <IconPlus size={16} />
