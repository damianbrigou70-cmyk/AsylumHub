...
const STAGES: { id: string; label: string; hex: string }[] = [
  { id: "lead", label: "Online", hex: "oklch(0.78 0.18 145)" },
  { id: "mql", label: "Patrol", hex: "oklch(0.78 0.15 200)" },
  { id: "sql", label: "Faction", hex: "oklch(0.74 0.17 250)" },
  { id: "opp", label: "Wanted", hex: "oklch(0.7 0.2 330)" },
  { id: "won", label: "Veteran", hex: "oklch(0.7 0.2 290)" },
];
...
function LeadsPage() {
  ...
  const loadOnlinePlayers = async () => {
    const result = await getOnlinePlayers({ data: { accessToken: session?.access_token } });
    setUnavailableServers(result.unavailable.map((item) => item.server));
    return result.players.map((player: OnlinePlayer) => ({
      id: player.id,
      full_name: player.name,
      email: null,
      company: player.server,
      title: "Online survivor",
      stage: "lead",
      source: player.server,
      channel: "players",
      notes: null,
      last_activity_on: player.lastSeen,
      created_at: player.lastSeen,
      is_sample: false,
    } satisfies Contact));
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setAllRows(await loadOnlinePlayers());
      } catch (loadError) {
        if (!cancelled) {
          setError(loadError instanceof Error ? loadError.message : "Could not load live players");
          setAllRows([]);
        }
      }
    })();
    const timer = window.setInterval(() => { void loadOnlinePlayers().then(setAllRows).catch(() => undefined); }, 30_000);
    return () => {
      cancelled = true;
      window.clearInterval(timer);
    };
  }, [reloadTick, session?.access_token]);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="flex items-start gap-4">
          <PageHexBadge hue={145} size={26} icon={<IconCampaign size={22} />} aria-label="Online players" />
          <div>
            <div className="text-xs uppercase tracking-[0.2em] text-muted-foreground">DayZ server</div>
            <h1 className="mt-1 font-display text-4xl tracking-tight">
              Online players

            </h1>
            <p className="mt-2 max-w-xl text-sm text-muted-foreground">
              Find survivors online, inspect their server stats, and review faction and linked-account details.
            </p>
          </div>
        </div>
      </div>
      ...
      {unavailableServers.length > 0 && (
        <div className="rounded-xl border border-amber-400/30 bg-amber-400/10 px-4 py-3 text-xs text-amber-100">
          Live logs unavailable for {unavailableServers.join(" and ")}. Other connected servers are still shown.
        </div>
      )}
      ...
      <GlassPanel className="p-3">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4">
          <Input
            placeholder="Search players, factions, or accounts…"
            value={q}
        ...
        {hasFilter && (
          <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
            <span>
              {rows?.length ?? 0} player{(rows?.length ?? 0) === 1 ? "" : "s"}{" "}
      ...
      {error ? (
        <PanelError
          message={`Couldn't load online players — ${error}`}
          onRetry={reload}
        />
      ) : rows === null ? (
        ...
        <GlassPanel className="overflow-hidden p-0">
          <div className="grid grid-cols-[1.8fr_1.2fr_0.9fr_0.7fr] gap-3 border-b border-glass-border/60 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
            <div>Player</div>
            <div>Server</div>
            <div>Status</div>
            ...
            {rows.map((c) => {
              return (
                <li
                  ...
                  className="grid cursor-pointer grid-cols-[1.8fr_1.2fr_0.9fr_0.7fr] items-center gap-3 px-4 py-3 text-sm transition-colors hover:bg-foreground/[0.04]"
                >

                  <div className="min-w-0">
                    <div className="flex items-center gap-2 truncate text-foreground/90">
                      <span className="size-2 shrink-0 rounded-full bg-emerald-400 shadow-[0_0_10px_oklch(0.78_0.18_145/0.8)]" />
                      {c.full_name}
                    </div>
                  </div>
      ...
      >
        <AlertDialogContent className="border-glass-border bg-background/95 backdrop-blur-xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {pendingDelete?.full_name} will be removed from the player list. This
              can't be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          <>
            ...
            <div className="px-6 pb-5">
              ...
              <dl className="divide-y divide-glass-border/40 border-y border-glass-border/40">
                <DetailRow label="Status" value="Online" />
                <DetailRow label="Server" value={contact.company} />
                <DetailRow label="Source" value="Live FTP server log" />
                <DetailRow label="Last seen" value={formatDateLong(contact.last_activity_on)} />
                <DetailRow
                  label="First seen"
              ...
              <div className="mt-5 flex items-center justify-end gap-2">
                ...
                {contact.email && (
                  ...
                  >
                    Message <IconArrowRight className="h-3 w-3" />
                  </a>
                )}
              </div>
            </div>
    ...
    <GlassPanel className="relative overflow-hidden p-10 text-center">
      <div className="mx-auto max-w-md space-y-3">
        <h2 className="font-display text-xl">
          {hasFilter ? "No matching players" : "No online players"}
        </h2>
        <p className="text-sm text-muted-foreground">
          {hasFilter
            ? "Try widening the filters above — your data is just hiding behind a narrow view."
            : "Players will appear here when the DayZ server reports them online."}
        </p>
        ...
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            {isEdit
              ...
              : "Add a player to the server roster."}
          </DialogDescription>
        </DialogHeader>
